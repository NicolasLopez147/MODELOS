...
#define COLA_RENTA 3
#define COLA_BUS 4
#define COLA_PUERTA_BUS 5
#define COLA_SUBIR_BUS 6
...
int   numero_parada, ruta, tipo_llegada, duracion_simulacion, puestos_ocupados,
      estado_paradas[NUMERO_PARADAS+1], bandera, numero_clientes_atendidos;
...
void inicializarSimulacion(){
    ...
    probabilidad_bajar_terminal[EVENTO_LLEGADA_TERMINAL_1] = 0.583;
    probabilidad_bajar_terminal[EVENTO_LLEGADA_TERMINAL_2] = 1.0;
...
void subir_bus(int cola)  /* Arrival event function. */
{
    list_remove(FIRST, cola);
    if (list_size[COLA_PUERTA_BUS] == 1) {
        transfer[1] = sim_time;
    }
    else {
        sampst(0.0, SAMPST_DELAYS);
        ++numero_clientes_atendidos;
        list_file(FIRST, COLA_PUERTA_BUS);
        event_schedule(sim_time + uniform(media_estre_descarga[3],media_estre_descarga[4],
                STREAM_SUBIR_DESCARGA),EVENTO_DESCARGA);
    }
}
/*
...
void bajar_bus(int evento)  /* Departure event function. */
{
    if (bandera == 1){
        if (ruta == evento){
            list_remove(FIRST, COLA_BUS);
        ...
        if (ruta == evento){
            ...
            sampst(sim_time - transfer[1], SAMPST_DELAYS);
            ++numero_clientes_atendidos;
            event_schedule(sim_time + uniform(media_estre_descarga[1],media_estre_descarga[2],
                    STREAM_BAJAR_DESCARGA),EVENTO_DESCARGA);
        }

    }
}

void descarga_bus()  /* Event function for departure of a job from a particular
                      station. */
{
    ...
    if (ruta == EVENTO_LLEGADA_TERMINAL_1){
        ...
        } else if (list_size[EVENTO_LLEGADA_TERMINAL_1] != 0 && puestos_ocupados <= CAPACIDAD_BUS){
            subir_bus(COLA_TERMINAL_1);
